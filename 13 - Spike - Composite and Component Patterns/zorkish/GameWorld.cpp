...
void GameWorld::movePlayerToSublocation(string subLocName)
{
    Location* mainLoc = _player->getLocation();
    for (auto& subloc : mainLoc->getSublocations())
    {
        if (subloc->getName() == subLocName)
        {
            _player->enterSublocation(subloc);
            return;
        }
    }
...
{
    Location* currentLocation = _player->getCurrentLocation();
    ...
    if (!sublocations.empty())
    {
        ...
        for (Location* sublocation : sublocations)
        {
            ...
            // For entities in sublocation
            Entity* entity = sublocation->findEntityByName("Chest");
            if (entity)
            {
